...
class ParallelVarPatchEmbed(nn.Module):
    ...
    def __init__(self, max_vars: int, img_size, patch_size, embed_dim, norm_layer=None, flatten=True):
        ...
        self.grid_size = (img_size[0] // self.patch_size[0], img_size[1] // self.patch_size[1]) # type: ignore
        ...
        self.proj_weights = nn.Parameter(grouped_weights) # type: ignore
        grouped_biases = torch.stack([_get_conv2d_biases(embed_dim) for _ in range(max_vars)], dim=0)
        self.proj_biases = nn.Parameter(grouped_biases) # type: ignore
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
        self.reset_parameters()

    def reset_parameters(self):
        for idx in range(self.max_vars):
            nn.init.kaiming_uniform_(self.proj_weights[idx], a=math.sqrt(5)) # type: ignore
    ...
    def forward(self, x, vars=None):
        ...
        weights = self.proj_weights[vars].flatten(0, 1) # type: ignore
        biases = self.proj_biases[vars].flatten(0, 1) # type: ignore

        groups = len(vars)
        proj = F.conv2d(x, weights, biases, groups=groups, stride=self.patch_size) # type: ignore
        if self.flatten:
