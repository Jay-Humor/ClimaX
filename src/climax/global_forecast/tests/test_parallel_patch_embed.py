...
def test_parallel_patch_embed():
    ...
    assert serial_model.token_embeds[0].num_patches == parallel_model.num_patches # type: ignore
    assert len(serial_model.token_embeds) == len(parallel_model.token_embeds.proj_weights) # type: ignore
    for i in range(len(serial_model.token_embeds)): # type: ignore
        serial_model.token_embeds[i].proj.weight.data = parallel_model.token_embeds.proj_weights[i].data # type: ignore
        serial_model.token_embeds[i].proj.bias.data = parallel_model.token_embeds.proj_biases[i].data # type: ignore
    ...
    for vars in test_vars:
        ...
        for i in range(len(serial_var_ids)):
            id = serial_var_ids[i]
            embeds.append(serial_model.token_embeds[id](x[:, i : i + 1, :, :])) # type: ignore
