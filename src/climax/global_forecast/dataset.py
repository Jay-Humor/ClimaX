    ...
    def __iter__(self):
        ...
        if worker_info is None:
            ...
            if self.multi_dataset_training:
                num_nodes = int(os.environ.get("NODES", None)) # type: ignore
        ...
        for idx in range(iter_start, iter_end):
            ...
            data = np.load(path)
            yield {k: data[k] for k in self.variables}, self.variables, self.out_variables # type: ignore
...
class Forecast(IterableDataset):
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            x = np.concatenate([data[k].astype(np.float32) for k in data.keys()], axis=1) # type: ignore
            x = torch.from_numpy(x)
            y = np.concatenate([data[k].astype(np.float32) for k in out_variables], axis=1) # type: ignore
